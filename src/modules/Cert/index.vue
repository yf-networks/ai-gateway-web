...
<template>
    <div>
        <Button class="add-button" type="primary" size="small" @click="onAdd">{{
            $t('com.createX', { obj: $t('cert.name') })
        }}</Button>
        <div class="cert-isdefult">
            <span>{{ $t('cert.globalDefaultCert') }}：</span>
            <Select v-model="showCertIsDefult" class="show-cert-isdefult" @on-change="onChangeCert">
                <Option v-for="item in tableData" :key="item.cert_name" :value="item.cert_name">{{
                    item.cert_name
                }}</Option>
            </Select>
        </div>
        ...
        >
            <Create v-if="isHiden" @submit="submitData" />
        </Drawer>
            ...
            columns: [
                ...
                },
                {
                    ...
                    render(h, params) {
                        return (
                            <span>
                                {params.row.cert_file_name ? params.row.cert_file_name : '-'}
                            </span>
                        );
                    }
                },
                        ...
                        return (
                            <span>
                                {params.row.expired_date
                                    ? moment(params.row.expired_date).format('YYYY-MM-DD HH:mm:ss')
                                    : '-'}
                            </span>
                        );
                    }
    ...
    methods: {
        ...
        onChangeCert(item) {
            ...
            this.$Modal.confirm({
                ...
                content:
                    this.$t('cert.tipSumitCertFile') +
                    certData.cert_name +
                    this.$t('cert.tipCofigDefault') +
                    '?',
                onOk: () => {
        ...
        onDel(data) {
            this.$Modal.confirm({
                ...
                title: this.$t('com.informationTips'),
                content:
                    this.$t('com.confirmDelX', { obj: this.$t('cert.name') }) +
                    data.cert_name +
                    '?',
                onOk: () => {
                    this.$request({
                        url: this.$urlFormat('certificates/{cert_name}', {
