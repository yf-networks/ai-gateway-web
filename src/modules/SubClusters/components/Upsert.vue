        ...
        <Form ref="formData" :model="formData" :rules="ruleValidate" label-position="top">
            <FormItem :label="$t('com.nameX', { obj: $t('subCluster.name') })" prop="name">
                <Input
                    ...
                    :placeholder="
                        $t('com.tipEnterX', {
                            obj: $t('com.nameX', { obj: $t('subCluster.name') })
                        })
                    "
                />
            </FormItem>
            ...
            <FormItem class="drawer-footer">
                ...
                <Button class="reset-btn" @click="handleReset('formData')">{{
                    $t('com.reset')
                }}</Button>
            </FormItem>
    ...
    data() {
        ...
        const validateName = (rule, value, callback) => {
                ...
                callback(
                    new Error(
                        this.$t('com.tipEnterX', {
                            obj: this.$t('com.nameX', { obj: this.$t('subCluster.name') })
                        })
                    )
                );
                return;
            ...
            if (this.subClustersNames.indexOf(value) !== -1) {
                callback(
                    new Error(this.$t('com.tipAlreadyExistsX', { obj: this.$t('subCluster.name') }))
                );
                return;
            }
            if (!ClustersNameRegCheck(value)) {
                callback(new Error(this.$t('subCluster.tipNameRule')));
                return;
            }
            callback();
        };
